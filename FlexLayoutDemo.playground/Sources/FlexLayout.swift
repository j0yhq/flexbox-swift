...
public struct FlexLayout: Layout {
    ...
    public func sizeThatFits(
        ...
        // When a finite size is proposed, fill it — a flex container acts like a
        // block box that accepts its parent's allocation (CSS default behavior).
        let contentW = result.size.width  + pad.leading + pad.trailing
        let contentH = result.size.height + pad.top + pad.bottom
        cache.containerSize = CGSize(
            width:  proposal.width.flatMap  { $0.isFinite ? max(contentW, $0) : nil } ?? contentW,
            height: proposal.height.flatMap { $0.isFinite ? max(contentH, $0) : nil } ?? contentH
        )
    ...
    private func computeLayout(
        ...
        let rawItems: [RawItem] = flowIndices.map { idx in
            ...
            var shrink = sv[FlexShrinkKey.self]
            ...
            if case .minContent = mainResolved {
                ...
                case .auto:
                    if let explicit = mainExplicit {
                        ...
                        let nat = sv.sizeThatFits(.unspecified)
                        basisMain = isRow ? nat.width : nat.height
                    }
                case .points(let n):
                    basisMain = max(0, n)
                case .fraction(let f):
