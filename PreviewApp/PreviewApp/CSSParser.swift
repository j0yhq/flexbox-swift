...
struct CSSParser {
    ...
    static func parse(_ input: String) -> ParsedCSS {
        ...
        let blocks = extractBlocks(from: css)
        ...
        for (selector, decls) in blocks {
            ...
            if isContainerRule && !rootFound {
                ...
                var selfItem = ParsedItem(selector: selector)
                if hasItem {
                ...
                if let rv = props["--repeat"], let n = Int(rv.trimmingCharacters(in: .whitespaces)), n > 1 {
                    repeatCounts[selector] = n
                }

            } else if isDisplayBlock || isDisplayInline {
                // display: block or inline — treat as a non-flex item with display mode
                let count = Int(props["--repeat"]?.trimmingCharacters(in: .whitespaces) ?? "") ?? 1
                let parentSel = findParentSelector(for: selector, among: nestedSelectors)
                for i in 0..<max(count, 1) {
                    var item = ParsedItem(selector: count > 1 ? shortLabel(selector, index: i + 1) : selector)
                    item.display = isDisplayBlock ? .block : .inline
            ...
            } else if hasItem || props["--repeat"] != nil {
                // Pure flex item rule (possibly repeated)
                let count = Int(props["--repeat"]?.trimmingCharacters(in: .whitespaces) ?? "") ?? 1
                let parentSel = findParentSelector(for: selector, among: nestedSelectors)
                for i in 0..<max(count, 1) {
                    var item = ParsedItem(selector: count > 1 ? shortLabel(selector, index: i + 1) : selector)
                    applyItemProps(props, to: &item, errors: &result.errors)
                    if let p = parentSel {
                        nestedItems[p, default: []].append(item)
                    } else {
                        result.items.append(item)
    ...
    private static func extractBlocks(from css: String) -> [(String, String)] {
        var results: [(String, String)] = []
        var i = css.startIndex

        while i < css.endIndex {
            guard let open  = css[i...].firstIndex(of: "{") else { break }
            guard let close = css[open...].firstIndex(of: "}") else { break }
            ...
            if !selector.hasPrefix("@") && !selector.isEmpty {
