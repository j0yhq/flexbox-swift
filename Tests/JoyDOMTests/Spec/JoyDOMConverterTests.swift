...
/// Unit 4 — `JoyDOMConverter.convert(_:)` translates a `Spec`
...
final class JoyDOMConverterTests: XCTestCase {
    ...
    func testEmptySpecProducesEmptyCSSAndOneRootEntry() {
        let spec = Spec(layout: Node(type: "div", props: NodeProps(id: "r")))
    ...
    func testInlineStyleCSSStandaloneMatchesConverterOutput() {
        ...
        let payload = JoyDOMConverter.convert(Spec(layout: layout))
